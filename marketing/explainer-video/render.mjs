// Renders a film frame by frame from the seekable timeline, then mixes the
// narration, the ambient bed and the effects, and muxes it all into one MP4.
//
//   node render.mjs en                      the English explainer
//   node render.mjs bn --film midnight      the Bangla cut of a short ad (films/<name>/)
...
const lang = positional[0] === "bn" ? "bn" : "en";
const film = opt("--film") || "explainer";
const base = film === "explainer" ? "." : path.join("films", film);
if (!fs.existsSync(path.join(base, "durations.json"))) { console.error(`no ${base}/durations.json — run: node tts.mjs --film ${film}`); process.exit(1); }
...
await page.goto(`http://127.0.0.1:${PORT}/marketing/explainer-video/timeline.html?lang=${lang}&film=${film}`);
...
console.log(`timeline ${tag}: ${info.total.toFixed(1)}s, ${info.scenes.length} scenes, ${info.cues.length} cues`);
fs.writeFileSync(path.join(base, `timeline-${lang}.json`), JSON.stringify(info, null, 1));
...
if (stills) {
  ...
  for (const s of stills.split(",")) {
    ...
    fs.writeFileSync(path.join("stills", `${tag}-${String(t).padStart(5, "0")}.png`), await shot("png"));
    console.log("still", t);
  }
...
const bedFile = `build/bed-${tag}.wav`, sfxFile = `build/sfx-${tag}.wav`;
{
  const [L, R] = bed(info.total + 1); wav(bedFile, L, R);
  const [sL, sR] = sfxTrack(info.cues, info.total + 1); wav(sfxFile, sL, sR);
}
...
const inputs = ["-i", bedFile, "-i", sfxFile];
...
const mixFile = `build/mix-${tag}.m4a`;
...
// ── Video ──────────────────────────────────────────────────────────────────
const videoFile = `build/video-${tag}.mp4`;
...
const out = (from || to) ? `tellmoreai-${tag}-slice.mp4` : `tellmoreai-${tag}.mp4`;
...
// poster: the brand frame of the explainer; the closing frame of an ad
const posterAt = film === "explainer" ? (info.scenes[1]?.t0 ?? 12) + 6 : (info.scenes[info.scenes.length - 1]?.t0 ?? 0) + 3.5;
execFileSync("ffmpeg", ["-y", "-ss", String(Math.min(dur - 1, Math.max(0, posterAt - from))), "-i", out, "-frames:v", "1", "-update", "1", "-q:v", "2", out.replace(/\.mp4$/, "-poster.jpg")], { stdio: ["ignore", "ignore", "inherit"] });
